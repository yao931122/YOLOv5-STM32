...
class Annotator:
    def __init__(self, im, line_width=None, font_size=None, font='Arial.ttf', pil=False):
        ...
        self.lw = line_width or max(int(round(sum(self.im.shape[:2]) / 2 * 0.003)), 2)

    def box_label(self, box, label='', color=(128, 128, 128), txt_color=(255, 255, 255)):
        # 用純 OpenCV 畫出完美的辨識方框與文字標籤
        p1, p2 = (int(box[0]), int(box[1])), (int(box[2]), int(box[3]))
        cv2.rectangle(self.im, p1, p2, color, thickness=self.lw, lineType=cv2.LINE_AA)
        if label:
            tf = max(self.lw - 1, 1)  # font thickness
            sf = self.lw / 3  # font scale
...
# 補上可能會被呼叫到的 colors 函數
def colors(i, bgr=False):
    hex_colors = ('FF3838', '2C99A8', '00E2F6', 'FFB21D', 'CFD231', '48F90A', 'FD349C', '2385FF')
    palette = [tuple(int(h[i:i+2], 16) for i in (0, 2, 4)) for h in hex_colors]
    c = palette[i % len(palette)]
    return (c[2], c[1], c[0]) if bgr else c

# 補上 save_one_box 函數防止 detect.py 崩潰
def save_one_box(xyxy, im, file=Path('im.jpg'), gain=1.02, pad=10, square=False, BGR=False, save=True):
    pass
